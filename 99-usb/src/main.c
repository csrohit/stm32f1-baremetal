...
static usbd_respond cdc_control(usbd_device *dev, usbd_ctlreq *req, usbd_rqc_callback *callback)
{
...
#ifdef ENABLE_HID_COMBO
    if (((USB_REQ_RECIPIENT | USB_REQ_TYPE) & req->bmRequestType) == (USB_REQ_INTERFACE | USB_REQ_CLASS) && req->wIndex == 2)
    {
        switch (req->bRequest)
        {
    ...
    if (((USB_REQ_RECIPIENT | USB_REQ_TYPE) & req->bmRequestType) == (USB_REQ_INTERFACE | USB_REQ_STANDARD) && req->wIndex == 2 && req->bRequest == USB_STD_GET_DESCRIPTOR)
    {
        switch (req->wValue >> 8)
        {
            case USB_DTYPE_HID:
...
static void clock_init(void)
{
    ...
    // 2. Configure Flash latency and prefetch buffer
    FLASH->ACR |= FLASH_ACR_PRFTBE; // Enable prefetch buffer
    FLASH->ACR &= ~FLASH_ACR_LATENCY;
    FLASH->ACR |= FLASH_ACR_LATENCY_2; // 2 wait states for 72MHz

    // 3. Set HCLK (AHB), PCLK1, PCLK2 prescalers
    RCC->CFGR |= RCC_CFGR_HPRE_DIV1;  // HCLK = SYSCLK
    RCC->CFGR |= RCC_CFGR_PPRE1_DIV2; // PCLK1 = HCLK/2 (max 36 MHz)
    RCC->CFGR |= RCC_CFGR_PPRE2_DIV1; // PCLK2 = HCLK

    // 4. Configure PLL: PLLCLK = HSE * 9 = 8 MHz * 9 = 72 MHz
    ...
    // 5. Set USB prescaler: USBPRE = 0 => PLLCLK/1.5 = 48 MHz
    RCC->CFGR &= ~RCC_CFGR_USBPRE; // 0: PLLCLK/1.5

    // 6. Enable PLL and wait for it to be ready
    RCC->CR |= RCC_CR_PLLON;
...
int main(void)
{
    ...
    udev.config_callback     = cdc_setconf;
    udev.control_callback    = cdc_control;
    udev.descriptor_callback = cdc_getdesc;
