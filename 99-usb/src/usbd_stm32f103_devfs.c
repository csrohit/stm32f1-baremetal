...
static uint16_t get_next_pma(uint16_t sz)
{
    ...
    {
        pma_table *tbl = EPT(i);
        if ((tbl->tx.addr) && (tbl->tx.addr < _result))
            _result = tbl->tx.addr;
        if ((tbl->rx.addr) && (tbl->rx.addr < _result))
            _result = tbl->rx.addr;
    }
...
static void ep_setstall(uint8_t ep, bool stall)
{
    ...
    if (USB_EP_ISOCHRONOUS == (*reg & USB_EP_T_FIELD))
        return;
...
static void enable(bool enable)
{
    if (enable)
    {
        ...
        RCC->APB1RSTR &= ~RCC_APB1RSTR_USBRST;
        USB->CNTR = USB_CNTR_CTRM | USB_CNTR_RESETM | USB_CNTR_ERRM | USB_CNTR_SOFM | USB_CNTR_SUSPM | USB_CNTR_WKUPM;
    }
...
__attribute__((externally_visible)) const struct usbd_driver usbd_devfs = {
    getinfo, enable, connect, setaddr, ep_config, ep_deconfig, ep_read, ep_write, ep_setstall, ep_isstalled, evt_poll, get_frame, get_serialno_desc,
};
